...
class RanSize {
    constructor(id, controls = {}) {
        ...
        this.ranCol = () => `rgb( ${Math.floor(Math.random() * 255) + 1} ,${Math.floor(Math.random() * 255) + 1} ,${Math.floor(Math.random() * 255) + 1})`;
        this.coolPattern = () => {
            this.i === (this.controls.value.length) - 1 ? this.i = 0 : this.i += 1;
            return this.controls.value[this.i]
        };
        this.rotateText = () => (Math.random() * (10 - (-10))) + (-10)
    }
    ...
    apply() {
        ...
        this.getText.split('').forEach(letter => {
            this.final += `<span class = "${this.controls.class}" style = "
                ...
                };${this.controls.colors === true ? `
                color: ${this.ranCol()};` : ''}${this.controls.rotate === true ? `
